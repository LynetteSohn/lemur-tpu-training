...
from xtpu.jax_utils import float_tensor_to_dtype, tree_apply

# from smart_open import open as smart.open


def resize_token_embeddings(token_embeddings, num_new_token_embeddings, axis):
    token_embeddings_avg = jnp.mean(token_embeddings, axis=axis)
    token_embeddings_avg = jnp.expand_dims(token_embeddings_avg, axis=axis)
    new_token_embeddings = jnp.concatenate(
        (
            token_embeddings,
            jnp.repeat(token_embeddings_avg, num_new_token_embeddings, axis=axis),
        ),
        axis=axis,
    )
...
class StreamingCheckpointer:
    ...
    def save_checkpoint(self, train_state, filename, gather_fns=None):
        path = os.path.join(self.checkpoint_dir, filename) if self.enable else "/dev/null"
    ...
    def save_train_state_to_file(train_state, path, gather_fns=None, float_dtype=None):
        ...
        with mlxu.open_file(path, "wb") as fout:
            # with smart.open(path, "wb") as fout:
    ...
    def save_all(self, train_state, gather_fns, metadata=None, dataset=None, milestone=False):
        ...
        if milestone:
            ...
            self.save_checkpoint(checkpoint_state, f"{checkpoint_name}_{step}", checkpoint_gather_fns)
        else:
    ...
    def load_checkpoint(path, target=None, shard_fns=None, remove_dict_prefix=None):
        ...
        with mlxu.open_file(path) as fin:
            ...
            for key, value in tqdm(unpacker, desc="Loading checkpoint"):
                key = tuple(key)
                if remove_dict_prefix is not None:
                ...
                if (
                    key == ("transformer", "wte", "embedding")
                    and flattened_target[("transformer", "wte", "embedding")].shape[0] > tensor.shape[0]
                ):
                    pprint.pprint(f"Resizing token embeddings from {tensor.shape} to {flattened_target[key].shape}")
                    num_new_token_embeddings = (
                        flattened_target[("transformer", "wte", "embedding")].shape[0] - tensor.shape[0]
                    )
                    pprint.pprint(f"Num new token embeddings: {num_new_token_embeddings}")
                    tensor = resize_token_embeddings(tensor, num_new_token_embeddings, 0)

                if key == ("lm_head", "kernel") and flattened_target[("lm_head", "kernel")].shape[1] > tensor.shape[1]:
                    pprint.pprint(f"Resizing lm head kernel from {tensor.shape} to {flattened_target[key].shape}")
                    num_new_token_embeddings = flattened_target[("lm_head", "kernel")].shape[1] - tensor.shape[1]
                    tensor = resize_token_embeddings(tensor, num_new_token_embeddings, 1)

                if (
                    key[:2] == ("transformer", "h")
                    and (key[-3:] == ("attention", "wk", "kernel") or key[-3:] == ("attention", "wv", "kernel"))
                    and flattened_target[key].shape[-1] < tensor.shape[-1]
                ):
                    pprint.pprint(
                        f"Resizing attention kernels {key} from {tensor.shape} to {flattened_target[key].shape}"
                    )
                    num_key_value_groups = tensor.shape[-1] // flattened_target[key].shape[-1]
                    tensor = tensor.reshape(tensor.shape[:-1] + (num_key_value_groups, flattened_target[key].shape[-1]))
                    tensor = jnp.mean(tensor, axis=-2)
                if shard_fns is not None:
    ...
    def load_trainstate_checkpoint(
        ...
        params_target = trainstate_target.params["params"] if trainstate_target is not None else None

        params_shard_fns = trainstate_shard_fns.params["params"] if trainstate_shard_fns is not None else None
        ...
        if load_type == "trainstate":
            ...
            pprint.pprint("Loading train state from: " + load_path)
            ...
            pprint.pprint("Loading train state params from: " + load_path)
            ...
            pprint.pprint("Loading params from: " + load_path)
