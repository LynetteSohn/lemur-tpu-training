...
from xtpu.model import FlaxLLaMAForCausalLM, LLaMAConfig
from xtpu.serving import LMServer
...
def main(argv):
    ...
    prefix_tokenizer = LLaMAConfig.get_tokenizer(FLAGS.tokenizer, truncation_side="left", padding_side="left")
    tokenizer = LLaMAConfig.get_tokenizer(FLAGS.tokenizer, truncation_side="right", padding_side="right")

    with jax.default_device(jax.devices("cpu")[0]):
        llama_config = LLaMAConfig.load_config(FLAGS.load_llama_config)
        _, params = StreamingCheckpointer.load_trainstate_checkpoint(FLAGS.load_checkpoint, disallow_trainstate=True)

        hf_model = FlaxLLaMAForCausalLM(
            llama_config, input_shape=(1, FLAGS.seq_length), seed=FLAGS.seed, _do_init=False
        )

    model_ps = match_partition_rules(LLaMAConfig.get_partition_rules(), params)
    shard_fns, _ = make_shard_and_gather_fns(model_ps, get_float_dtype_by_name(FLAGS.dtype))

    @partial(pjit, in_shardings=(model_ps, PS(), PS()), out_shardings=(PS(), PS(), PS()))
    def forward_loglikelihood(params, rng, batch):
        ...
        match_count = jnp.sum((jnp.argmax(logits, axis=-1) == output_tokens) * output_mask, axis=-1)
        total = jnp.sum(output_mask, axis=-1)
        is_greedy = match_count == total
        return loglikelihood, is_greedy, rng_generator()

    @partial(pjit, in_shardings=(model_ps, PS(), PS(), PS()), out_shardings=(PS(), PS()))
    def forward_generate(params, rng, batch, temperature):
        ...
        output = hf_model.generate(
            ...
            logits_processor=FlaxLogitsProcessorList([FlaxTemperatureLogitsWarper(temperature)]),
    ...
    class ModelServer(LMServer):
        ...
        def loglikelihood(prefix_text, text):
            ...
            bos_tokens = np.full((output_tokens.shape[0], 1), tokenizer.bos_token_id, dtype=np.int32)
            ...
            bos_mask = np.ones_like(input_mask[:, :1]) if FLAGS.add_bos_token else np.zeros_like(input_mask[:, :1])
        ...
        def loglikelihood_rolling(text):
            ...
            if output_tokens.shape[1] < FLAGS.seq_length:
                ...
                pad_tokens = np.full((batch_size, padding_length), tokenizer.pad_token_id, dtype=np.int32)
                output_tokens = np.concatenate([output_tokens, pad_tokens], axis=-1)
                pad_mask = np.zeros((batch_size, padding_length), dtype=inputs.attention_mask.dtype)
                attention_mask = np.concatenate([attention_mask, pad_mask], axis=-1)

            bos_tokens = np.full((batch_size, 1), tokenizer.bos_token_id, dtype=np.int32)
            input_tokens = np.concatenate([bos_tokens, output_tokens[:, :-1]], axis=-1)
            np.ones((batch_size, 1), dtype=inputs.attention_mask.dtype)
        ...
        def generate(text, temperature):
            ...
            with mesh:
                output, sharded_rng = forward_generate(params, sharded_rng, batch, temperature)
        ...
        def greedy_until(prefix_text, until, max_length):
            ...
            for pf, ut in zip(prefix_text, until):
                ...
                while total_length < max_length:
                    ...
                    batch = {"input_tokens": input_tokens, "attention_mask": attention_mask}
