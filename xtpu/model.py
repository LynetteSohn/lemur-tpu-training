...
from xtpu.jax_utils import get_gradient_checkpoint_policy, get_jax_mesh, with_sharding_constraint
...
def repeat_kv(hidden_states: jnp.ndarray, n_rep: int) -> jnp.ndarray:
    """
    This is a reimplemention of repeat_kv_torch using jax.numpy. The hidden states go from (batch,
    num_key_value_heads, seqlen, head_dim) to (batch, num_attention_heads, seqlen, head_dim)
    """
    batch, slen, num_key_value_heads, head_dim = hidden_states.shape
    if n_rep == 1:
        return hidden_states
    # JAX expand_dims and repeat operation
    hidden_states = jnp.expand_dims(hidden_states, axis=3)
    hidden_states = jnp.repeat(hidden_states, n_rep, axis=3)

    # JAX reshape operation
...
class FlaxLLaMAAttention(nn.Module):
    ...
    def __call__(
        ...
        xq = xq.reshape(xq.shape[:2] + (self.num_heads, self.head_dim))
        xk = xk.reshape(xk.shape[:2] + (self.num_key_value_heads, self.head_dim))
        xv = xv.reshape(xv.shape[:2] + (self.num_key_value_heads, self.head_dim))
...
class FlaxLLaMABlock(nn.Module):
    ...
    def setup(self) -> None:
        ...
        if self.config.remat_mlp != "":
            mlp_module = remat(
                FlaxLLaMAMLP, static_argnums=(1,), policy=get_gradient_checkpoint_policy(self.config.remat_mlp)
            )
