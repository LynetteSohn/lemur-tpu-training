...
class LMServer:
    ...
    def serve_loglikelihood(self, data: InferenceRequest):
        with self.lock:
            ...
            text = [self.config.prepend_to_text + t + self.config.append_to_text for t in data.text]
            ...
            for i in trange(0, len(text), self.config.batch_size, ncols=0):
                ...
                batch_log_likelihood, batch_is_greedy = self.loglikelihood(batch_prefix_text, batch_text)
    ...
    def serve_loglikelihood_rolling(self, data: InferenceRequest):
        with self.lock:
            ...
            for i in trange(0, len(text), self.config.batch_size, ncols=0):
                ...
                batch_log_likelihood, batch_is_greedy = self.loglikelihood_rolling(batch_text)
                batch_log_likelihood = self.to_list(batch_log_likelihood)
                batch_is_greedy = self.to_list(batch_is_greedy)
                log_likelihood.extend(batch_log_likelihood[:batch_size])
                is_greedy.extend(batch_is_greedy[:batch_size])
    ...
    def process_chat(self, prompt, context, temperature):
        context = (
            context + self.config.chat_user_prefix + prompt + self.config.chat_user_suffix + self.config.chat_lm_prefix
        )
    ...
    def create_chat_app(self):
        with gr.Blocks(analytics_enabled=False, title="EasyLM Chat") as gradio_chatbot:
            ...
            temp_slider = gr.Slider(label="Temperature", minimum=0, maximum=2.0, value=self.config.default_temperature)
            ...
            def model_fn(history, context, temperature):
                history[-1][1], new_context = self.process_chat(history[-1][0], context[0], temperature)
                return {
            ...
            regenerate.click(regenerate_fn, inputs=None, outputs=[msg, clear, send, regenerate], queue=False).then(
                model_fn,
                inputs=[chatbot, context_state, temp_slider],
                outputs=[msg, clear, send, chatbot, context_state, regenerate],
                queue=True,
            )
            clear.click(clear_fn, inputs=None, outputs=[chatbot, msg, context_state, regenerate], queue=False)
        ...
        if self.config.pre_compile != "":
            ...
            for task in pre_compile:
                if task == "loglikelihood":
                    ...
                    self.greedy_until(pre_compile_data, pre_compile_data, self.config.greedy_until_max_length)
                elif task == "chat":
...
class LMClient:
    ...
    def loglikelihood(self, prefix, text):
        ...
        batched_iterator = list(
            zip(self.batched(prefix, self.config.batch_size), self.batched(text, self.config.batch_size))
        )
    ...
    def loglikelihood_rolling(self, text):
        ...
        for batch_text in tqdm(batched_iterator, ncols=0):
            response = requests.post(
                urllib.parse.urljoin(self.config.url, "loglikelihood-rolling"), json={"text": batch_text}
            ).json()
            log_likelihood.extend(response["log_likelihood"])
            is_greedy.extend(response["is_greedy"])
        return log_likelihood, is_greedy
